...
post('/buy/([a-zA-Z0-9\-_]+)', function($id) {
  ...
  else if($method == 'freekassa') $cMethod = 'freekassa';
  ...
  if($method == 'qiwi') header("Location: /buy/qiwi/".$u["id"]);
    ...
    print('<form method="POST" action="https://primepayer.com/payment"><input name="shop" type="hidden"       value="'.$data['shop'].'"><input name="payment" type="hidden"     value="'.$data['payment'].'"><input name="amount" type="hidden"      value="'.$data['amount'].'"><input name="description" type="hidden" value="'.$data['description'].'"><input name="currency" type="hidden"    value="'.$data['currency'].'"><input name="sign"  type="hidden"       value="'.$sign.'"><button id="nextBTN">Продолжить оплату</button></form><script>document.getElementById("nextBTN").click();</script>');
  	...
  	$url = 'https://www.free-kassa.ru/merchant/cash.php?m='.FREEKASSA["id"].'&oa='.$price.'&o='.$u["id"].'&s='.md5(FREEKASSA["id"].':'.$price.':'.FREEKASSA["secret1"].':'.$u["id"]);
    header('Location: '.$url);
  }
...
// Отплавка товара 
function successPAY($id, $store, $value, $email, $coupon, $date) {
	$mysqli = database::connect();  $mysqli->set_charset("utf8");	
      // Выдача
    $content = '';
    $distribution = $mysqli->query("select gk.id, store.name, gk.key from `store` as store, `store_keys` as gk, `payments` as pay where pay.store = store.id and pay.id = '$id' and gk.store = store.id and gk.store = '$store' and gk.status = 0 limit ".$value);
    while($result = mysqli_fetch_array($distribution)):
      $key_id = $result["id"];
      $mysqli->query("UPDATE `store_keys` SET `status` = '1', `coupon` = '$coupon', `email` = '$email', `date` = '$date' WHERE `id` = '$key_id'");
      $content .= $result['name'].' - '.$result['key'].'<br />';
    endwhile; 
    phpmailer($email, 'Покупка на сайте ', $content);
    updateItems($store);
...
get('/api/yandex/success/([a-zA-Z0-9\-_]+)', function($id) {
  ...
  if(mysqli_num_rows($sql) != null) {
	...
	if(successPAY($pay_id, $u["store"], $u["value"], $u["email"], $u["coupon"], $u["date"])) header("Location: /pay/success");
  }
});
...
post('/api/qiwi/([a-zA-Z0-9\-_]+)', function($id){
  ...
  if(QIWI::pay(round($sum), $u["id"])) { 
	$pay_id = $u["id"];
	$mysqli->query("UPDATE `payments` SET `status` = '1' WHERE `id` = '$pay_id'");
	if(successPAY($pay_id, $u["store"], $u["value"], $u["email"], $u["coupon"], $u["date"])) {
...
post('/prime/success', function(){
  ...
  $sql = $mysqli->query("SELECT * FROM `payments` WHERE `method` = 'primepayer' and id = '$id' LIMIT 1");
  if(mysqli_num_rows($sql) != null) {
    $u = $sql->fetch_assoc();
    if($u["status"] == 1) if(successPAY($id, $u["store"], $u["value"], $u["email"], $u["coupon"], $u["date"])) header("Location: /pay/success");
    else header("Location: /");
  }
});
post('/prime/notify', function(){ 
    $ip = false;
    $list = ['109.120.152.109', '145.239.84.249'];
    for($i = 0; $i < count($list); $i++) if($list[$i] == $_SERVER['HTTP_CF_CONNECTING_IP']) $ip = true;
    if(!$ip) return alert('error', 'Ошибка при проверке IP PrimePayer');
	$secret = primepayer["secret"];
	$currency = '3';
	$id = $_POST["payment"];
	$mysqli = database::connect();
    $sql = $mysqli->query("SELECT * FROM `payments` WHERE `method` = 'primepayer' and id = '$id' LIMIT 1");
    if(mysqli_num_rows($sql) == null) return alert('error', 'Не удалось найти заказ #'.$id); 
    $u = $sql->fetch_assoc();
    if($u["status"] == 1) return alert('error', 'Вы уже оплатили заказ!');
    $sign = $_POST['sign'];
	unset($_POST['sign']);
	ksort($_POST,SORT_STRING);
	$signi = hash('sha256', implode(':', $_POST).':'.$secret);
	if($signi !== $sign) return alert('error', 'hash error');
	$mysqli->query("UPDATE `payments` SET `status` = '1' WHERE `id` = '$id'");
    return alert('success', true);
});

// Freekassa.Оплата
  post('/freekassa/result', function() {
      $ip = false;
      $list = ['136.243.38.147', '136.243.38.149', '136.243.38.150', '136.243.38.151', '136.243.38.189', '88.198.88.98', '136.243.38.108'];
      for($i = 0; $i < count($list); $i++) if($list[$i] == $_SERVER['HTTP_CF_CONNECTING_IP']) $ip = true;
      if(!$ip) return alert('error', 'Ошибка при проверке IP free-kassa');
      $mysqli = database::connect();
      $id = $_REQUEST['MERCHANT_ORDER_ID']; // ID Заказа
      $sql = $mysqli->query("SELECT * FROM `payments` WHERE `method` = 'freekassa' and id = '$id' LIMIT 1");
      if(mysqli_num_rows($sql) == null) return alert('error', 'Не удалось найти заказ #'.$id); 
      $u = $sql->fetch_assoc();
      if($u["status"] == 1) return alert('error', 'Вы уже оплатили заказ!');
      $sign = md5(FREEKASSA['id'].':'.$_REQUEST['AMOUNT'].':'.FREEKASSA["secret2"].':'.$id);
      if ($sign != $_REQUEST['SIGN']) return alert('error', 'wrong sign');
      $mysqli->query("UPDATE `payments` SET `status` = '1' WHERE `id` = '$id'");
      return alert('success', true);
  });
  get('/freekassa/success', function() {
      $id = $_SESSION["PAYID"];
      $mysqli = database::connect();
      $sql = $mysqli->query("SELECT * FROM `payments` WHERE `method` = 'freekassa' and id = '$id' LIMIT 1");
      if(mysqli_num_rows($sql) != null) {
