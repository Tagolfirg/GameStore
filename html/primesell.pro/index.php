...
if(OFFLINE == 1):
  return print(SITENAME["name"].' offline');
endif;
...
require(BASE_PATH.'/vendor/Nanite.php'); // Nanite
require(APP_PATH.'/web.php'); // routes

function tpl($args) {
  $empty = 'pages/'.$args[1].'.php';
  require(BASE_PATH."/resources/".$args[0].".layout.php");
}


class database {
      public static function connect() {
          return new mysqli(SQL["host"], SQL["user"], SQL["password"], SQL["database"]);
      }
...
function alert($status, $msg) {
  return print(json_encode(array('status' => $status, 'msg' => $msg)));
}
...
function phpmailer($to, $title, $content) {

  $message = '<html><head><meta name="viewport" content="width=device-width" /><meta http-equiv="Content-Type" content="text/html; charset=UTF-8" /><title>Получение заказа</title>
  ...
  </head><body class=""><table role="presentation" border="0" cellpadding="0" cellspacing="0" class="body"><tr><td>&nbsp;</td><td class="container"><div class="content"><table role="presentation" class="main">
  <tr><td class="wrapper"><table role="presentation" border="0" cellpadding="0" cellspacing="0"><tr><td>'.$content.'<hr /><center><p>Спасибо за покупки на нашем сайте '.main["name"].'</p></center></td></tr></table></td></tr></table><div class="footer"><table role="presentation" border="0" cellpadding="0" cellspacing="0"><tr><td class="content-block powered-by"><a href="http://'.$_SERVER['HTTP_HOST'].'">Перейти на сайт '.main["name"].'</a>.</td></tr></table></div></div></td><td>&nbsp;</td></tr></table></body></html>';


  $headers  = 'MIME-Version: 1.0' . "\r\n"; $headers .= 'Content-type: text/html; charset=utf-8' . "\r\n";
  $headers .= 'From: '.main["name"].' <noreply@'.$_SERVER['HTTP_HOST'].'>';
